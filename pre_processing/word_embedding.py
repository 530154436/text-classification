...
# -*- coding:utf-8 -*-
import os
from config import logger,SEG_DIR,MODEL_DIR
from gensim.models import word2vec, KeyedVectors
from gensim.models.word2vec import LineSentence
import multiprocessing
import os.path

def train_word2vec(seg_file, number_cores=8, sg=0, size=300, window=5, min_count=3, iter=100):
    '''
    训练词向量模型
    ...
    # 训练 skip-gram 模型
    model = word2vec.Word2Vec(LineSentence(seg_file), workers=number_cores,
...
if __name__ == '__main__':
    ...
    train_word2vec(seg_file, number_cores=8, sg=0, size=150, window=5, min_count=3, iter=50)
    train_word2vec(seg_file, number_cores=8, sg=0, size=50, window=5, min_count=3, iter=50)